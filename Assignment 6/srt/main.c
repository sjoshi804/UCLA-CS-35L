...
//Global Vars Init
int nthreads;
float floatRGBVal[width][height][3];
scene_t scene;

//Function that delimits work for each thread
void* workShare(void* numThread)
{
    Vec3 camera_pos;
    ...
    int start = (*(int*)numThread) * (width/nthreads);
    int end = (*(int*)numThread) == (nthreads-1) ? width : (*(int*)numThread + 1) * (width/nthreads);
    
    /* worksharing for every pixel */
    for( int px=start; px<end; px++ )
    {
	    ...
	    floatRGBVal[px][py][0] = gamma( pixel_color[0] ) * max_color;
	    floatRGBVal[px][py][1] = gamma( pixel_color[1] ) * max_color;
	    floatRGBVal[px][py][2] = gamma( pixel_color[2] ) * max_color;

	    /* enforce caps, replace with real gamma */
	    for( int i=0; i<3; i++)
	      floatRGBVal[px][py][i] = max( min(floatRGBVal[px][py][i], 255), 0);
...
main( int argc, char **argv )
{
    ...
    {
      threadID[i] = i;
      failure = pthread_create(&threads[i], NULL, workShare, &threadID[i]);
      if (failure)
      {
    ...
    for (i = 0; i < nthreads; i++)
    {
      ...
      if (failure)
      {
	fprintf(stderr, "Multithreading Error: Thread Joining Error(%d)\n",
		failure);
	exit(failure);
      }
    }
    ...
    for (int i = 0; i < width; i++)
    {
      ...
      {
	printf( "%.0f %.0f %.0f\n",
		floatRGBVal[i][j][0], floatRGBVal[i][j][1], floatRGBVal[i][j][2]);
      }
      printf("\n");
    }
    ...
    if(fclose(stdout) != 0 || ferror(stdout))
    {
